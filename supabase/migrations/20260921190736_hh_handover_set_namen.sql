-- Das Hohe Haus V24d, Nachtrag vom 22.09.2026 (aus der Prüfung von a9f1a9e).
-- hh_handover_set hat den Ausgang nur bei den Zeichenketten 'alex' und 'lea' getroffen. In den Daten stehen
-- aber „Alexander Dettke“ und „Lea Luce“; die Edge Function normalisiert seit jeher über whoNorm (enthält lea,
-- enthält alex, sonst Team). Ohne diesen Nachtrag landet ein Thema bei „team“ statt bei „alex“.
-- Außerdem galt ein gespeicherter Leerstring als frühere Vertretung; das führte zu einer Rückgabe, die keine war.

create or replace function public.hh_person_gate(p_name text, p_sonst text default 'team')
returns text
language sql
immutable
as $$
  select case
    when position('lea'  in lower(coalesce(p_name,''))) > 0 then 'lea'
    when position('alex' in lower(coalesce(p_name,''))) > 0 then 'alex'
    else p_sonst end;
$$;
comment on function public.hh_person_gate(text, text) is
  'Das Hohe Haus V24d: Ausgang zu einem Namen, gleiche Regel wie whoNorm in der Edge Function (enthält lea, enthält alex, sonst der Ersatzwert).';
...
begin
  ...
  if v_row.kind = 'thema' then
    if v_ampel = 'ruht' then
      ...
      where id = v_row.ref_id::uuid
      returning jsonb_build_object('id', id, 'gate', gate, 'owner_backup', owner_backup) into v_vorgang;
    elsif v_alt_vertretung is not null then
...
revoke all on function public.hh_person_gate(text, text) from public, anon, authenticated;
grant execute on function public.hh_person_gate(text, text) to service_role;
